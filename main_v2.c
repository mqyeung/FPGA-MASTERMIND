...
//   OLEDrgb_PutChar(&oledrgb, 4);
  ...
  // XGpio_Initialize(&output, XPAR_AXI_GPIO_1_DEVICE_ID);	//initialize output XGpio variable
   ...
   OLEDrgb_PutString(&oledrgb, "Game"); // Default color (green)
   OLEDrgb_SetCursor(&oledrgb, 1, 4);
   OLEDrgb_PutString(&oledrgb, "starting..."); // Default color (green)
   usleep(2000000);
   OLEDrgb_Clear(&oledrgb);
   int play_again = 1;
	...
	while (play_again == 1) {
	   ...
	   int unsubmitted = 1;
	   while(stage == 1){//game master enters code
		   OLEDrgb_Clear(&oledrgb);
		   OLEDrgb_SetCursor(&oledrgb, 1, 3);
		   OLEDrgb_PutString(&oledrgb, "Game master"); // Default color (green)
		   OLEDrgb_SetCursor(&oledrgb, 1, 4);
		   OLEDrgb_PutString(&oledrgb, "enter your"); // Default color (green)
		   OLEDrgb_SetCursor(&oledrgb, 1, 5);
		   OLEDrgb_PutString(&oledrgb, "code!"); // Default color (green)
		   usleep(2000000);

		   OLEDrgb_Clear(&oledrgb);
		   OLEDrgb_SetFontColor(&oledrgb, OLEDrgb_BuildRGB(200, 12, 44));
		   OLEDrgb_SetCursor(&oledrgb, 1, 1);
		   OLEDrgb_PutString(&oledrgb, "Enter code:"); // Default color (green)
	...
	//			   }
			   ...
			   if(button_data == 0b00100){
				   xil_printf("button 2 pressed. saving\n\r");
				   solution[0] = spot_a;
				   solution[1] = spot_b;
				   solution[2] = spot_c;
				   solution[3] = spot_d;
	   ...
	   usleep(200000);
	   if (intro){
		   OLEDrgb_SetFontColor(&oledrgb, OLEDrgb_BuildRGB(27, 181, 239));
		   OLEDrgb_SetCursor(&oledrgb, 0, 0);
		   OLEDrgb_PutString(&oledrgb, "Created by");
		   OLEDrgb_SetFontColor(&oledrgb, OLEDrgb_BuildRGB(200, 12, 44));
		   OLEDrgb_SetCursor(&oledrgb, 1, 2);
		   OLEDrgb_PutString(&oledrgb, "Matt");
		   OLEDrgb_SetCursor(&oledrgb, 1, 3);
		   OLEDrgb_PutString(&oledrgb, "Yeung");
		   OLEDrgb_SetCursor(&oledrgb, 1, 5);
		   OLEDrgb_PutString(&oledrgb, "Vincent");
		   OLEDrgb_SetCursor(&oledrgb, 1, 6);
		   OLEDrgb_PutString(&oledrgb, "Johnson");
		   usleep(4000000);
		   ...
		   OLEDrgb_PutString(&oledrgb, "Flip the"); // Default color (green)
		   OLEDrgb_SetCursor(&oledrgb, 0, 3);
		   OLEDrgb_PutString(&oledrgb, "switches"); // Default color (green)
		   OLEDrgb_SetCursor(&oledrgb, 0, 4);
		   OLEDrgb_PutString(&oledrgb, "to guess"); // Default color (green)
		   OLEDrgb_SetCursor(&oledrgb, 0, 5);
		   OLEDrgb_PutString(&oledrgb, "the code!"); // Default color (green)
		   usleep(4000000);
		   OLEDrgb_Clear(&oledrgb);
		   OLEDrgb_SetCursor(&oledrgb, 0, 0);
		   OLEDrgb_PutString(&oledrgb, "Each group"); // Default color (green)
		   OLEDrgb_SetCursor(&oledrgb, 0, 1);
		   OLEDrgb_PutString(&oledrgb, "of four"); // Default color (green)
		   OLEDrgb_SetCursor(&oledrgb, 0, 2);
		   OLEDrgb_PutString(&oledrgb, "switches is"); // Default color (green)
		   OLEDrgb_SetFontColor(&oledrgb, OLEDrgb_BuildRGB(0, 255, 0));
		   OLEDrgb_SetCursor(&oledrgb, 0, 4);
		   OLEDrgb_PutString(&oledrgb, "Red"); // Default color (green)
		   OLEDrgb_SetFontColor(&oledrgb, OLEDrgb_BuildRGB(0, 255, 255));
		   OLEDrgb_SetCursor(&oledrgb, 0, 5);
		   OLEDrgb_PutString(&oledrgb, "Yellow"); // Default color (green)
		   OLEDrgb_SetFontColor(&oledrgb, OLEDrgb_BuildRGB(0, 0, 255));
		   OLEDrgb_SetCursor(&oledrgb, 0, 6);
		   OLEDrgb_PutString(&oledrgb, "Green"); // Default color (green)
		   OLEDrgb_SetFontColor(&oledrgb, OLEDrgb_BuildRGB(255, 0, 0));
		   OLEDrgb_SetCursor(&oledrgb, 0, 7);
		   OLEDrgb_PutString(&oledrgb, "Blue"); // Default color (green)
		   usleep(4000000);
		   OLEDrgb_Clear(&oledrgb);
	   }
	   unsubmitted = 1;
	...
	//   };
	   ...
	   while(stage == 2){ //will keep looping as long as guess is not complete
		   OLEDrgb_SetFontColor(&oledrgb, OLEDrgb_BuildRGB(200, 12, 44));
	...
	//			   }
		   ...
		   //compare time
		   int guess_temp[4], solution_temp[4];
		   for (int i = 0; i < 4; i++){
			   guess_temp[i] = guess[i];
			   solution_temp[i] = solution[i];
		   }
		   int red = 0;
		   int white = 0;
		   for (int i = 0; i < 4; i++){
			   if (guess_temp[i] == solution_temp[i]){
				   guess_temp[i] = -1;
				   solution_temp[i] = -1;
				   red++;
			   }
		   ...
		   for (int i = 0; i < 4; i++){
			   for (int j = 0; j < 4; j++){
				   if (guess_temp[i] != -1 && solution_temp[j] != -1){
					   if (guess_temp[i] == solution_temp[j]){
						   guess_temp[i] = -1;
						   solution_temp[j] = -1;
						   white++;
					   }
		   ...
		   OLEDrgb_DrawRectangle(&oledrgb, 16, 40, 31, 55, OLEDrgb_BuildRGB(255, 255, 255), 1, 	OLEDrgb_BuildRGB(255, 255, 255));
		   char red_str[20];
